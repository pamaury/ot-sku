...
def create_branch(pargs, branch_sha, branch_name):
    create_ref(pargs, f"branch {branch_name}", f"refs/heads/{branch_name}", branch_sha)
...
def create_presign_release(pargs):
    ...
    # Create release. Note this tag does not use the final release tag
    # but instead has a _BUILD suffix to make it clear that this is not
    # the final release.
    release_tag = f"{pargs.tag}_BUILD"
    ...
    run_gh_or_exit(pargs, "cannot create release", [
        ...
        "-t", pargs.release_title or release_tag,
...
def create_presign_branch(pargs, release_tag):
    # Important note:
    ...
    # Get asset information so that we can update the archives in the extension.
    release_info = get_release_info(pargs, release_tag)
    ...
    # To avoid ambiguities between the git tag (created automatically when doing the
    # final release) and the release branch, we use a different name for the branch.
    branch_name = "release_{}".format(pargs.tag)
...
def create_postsign_release(pargs):
    ...
    # Get asset information so that we can update the archives in the extension.
    release_info = get_release_info(pargs, "{pargs.tag}_BUILD")

    # Get artifact list by looking at the released presigned items.
    artifact_info = get_release_asset_info(
        pargs, release_info, {
            art: ARTIFACTS[art]["presign_name"]
            for art in pargs.release_artifacts
        },
        allow_no_match=True,
    )

    print_info(pargs, "the following artifacts will be released: {}".format(
        ",".join(list(artifact_info.keys()))
    ))

    for art in artifact_info:
        # Run a signature test check.
        run_or_exit(pargs, f"cannot verify the signatures of {art}", [
            pargs.bazelisk, "test"] + pargs.bazel_opts + [
            "--test_output=streamed"] + [
                ARTIFACTS[art]["sig_test_label"] for art in artifact_info],
            cwd=pargs.ot_repo,
        )
...
def main(argv):
    ...
    if args.pre_sign:
        release_tag = create_presign_release(args)
        create_presign_branch(args, release_tag)
    if args.post_sign:
